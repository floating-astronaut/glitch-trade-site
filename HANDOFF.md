# Handoff prompt — glitch-trade-site

Copy everything below the `---` into the next session.

---

You are taking over the `glitch-trade-site` Astro upgrade at
`/home/support/glitch-trade-site/`. The previous session replaced the old static
...
5. **Deploy to Cloudflare Pages.** Build command `npm run build`, output dir `dist`, functions dir auto-detected from `functions/`. Domain: `trade.glitchexecutor.com`.
...
## Commands
```bash
cd /home/support/glitch-trade-site
