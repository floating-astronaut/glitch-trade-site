...
// Stroke is amber to match the Trade brand accent.
const cobraBadge = `
  <svg width="72" height="72" viewBox="0 0 24 24" fill="none" stroke="#f59e0b" stroke-width="1.5"
    ...
    <circle cx="10.5" cy="7.5" r="0.4" fill="#f59e0b" />
    <circle cx="13.5" cy="7.5" r="0.4" fill="#f59e0b" />
  </svg>`;

export function ogTemplate({
  eyebrow = 'Glitch Trade',
  title,
  subtitle = '9 AI models. One trading edge.',
  accent = '#f59e0b',
  bg = '#0a0a0f',
}: OgParams) {
  const markup = `
    <div style="
      ...
      background: ${bg};
      background-image: radial-gradient(circle at 80% 10%, rgba(245,158,11,0.18), transparent 55%),
                        radial-gradient(circle at 10% 90%, rgba(250,204,21,0.15), transparent 55%);
      color: #F5F7FA;
      padding: 72px 80px;
