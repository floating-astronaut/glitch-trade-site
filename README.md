...
- public website for the Glitch Trade brand
- positions the ensemble, Telegram delivery, and multi-exchange execution story
- contains the landing-page metadata, OG tags, legal disclaimers, and public-facing copy for the site
...
Glitch Trade is part of the broader **Glitch Executor** brand family alongside Glitch Grow and Glitch Edge.
...
| ----------------------------- | ----------- | -------------- | ---------------------------------------------------------------------------------------- |
| `PUBLIC_SITE_URL`             | build       | yes            | Canonical origin. CI sets it to `https://trade.glitchexecutor.com`.                       |
...
2. Domain: `trade.glitchexecutor.com`. Widget mode: **Managed**.
...
3. Environment variables — Production:
   - `PUBLIC_SITE_URL=https://trade.glitchexecutor.com`
...
6. Custom domain: `trade.glitchexecutor.com` → Pages will auto-provision a cert via CF DNS.
...
- `curl -I https://trade.glitchexecutor.com/` → `200`, long-cache headers on `/_astro/*` + `/assets/*`.
